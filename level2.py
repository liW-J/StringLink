        ...
        for data in txt:
            ...
            for num in range(lenthOfListLine):
                ...
                if lenthOfListLine > 10:
                    ...
                    if LevelOne>=2 and LevelTwo>=4:
                        break
                    else:
                        LevelOne,count,data=dig( num, LevelOne, data, key, word,count,2, result1,listLine)#一级词汇出2个
                        LevelTwo,count,data=dig( num, LevelTwo, data, key, word,count,4, result2,listLine)#二级词汇出4个
            ...
            if len(key)>4 or len(key)<2 :
                ...
                with open ("key/key2.csv", 'a', encoding='utf8', newline='') as f1:
                    f_csv = csv.writer(f1)
                    f_csv.writerow(key)
                file2.write(data)
                depart = department(listOfStr)
                with open("kind/kind2.csv", 'a', encoding='utf8', newline='') as f3:
